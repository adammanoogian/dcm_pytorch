% run_spm_spectral_dcm.m -- SPM12 spectral DCM estimation batch script
%
% Usage: matlab -batch "run_spm_spectral_dcm"
%   or:  matlab -batch "run('validation/matlab_scripts/run_spm_spectral_dcm.m')"
%
% Reads:  validation/data/spectral_dcm_input.mat  (DCM struct)
% Writes: validation/data/spectral_dcm_spm_results.mat (results struct)
%
% Key difference from task DCM: uses spm_dcm_fmri_csd (CSD analysis mode).
% SPM12 computes CSD from BOLD internally via MAR model.
% Source: Verified against spm_dcm_fmri_csd.m (local SPM12 install).

fprintf('=== SPM12 Spectral DCM Estimation ===\n');
...
if isempty(input_path)
    input_path = 'validation/data/spectral_dcm_input.mat';
end
output_path = getenv('DCM_OUTPUT_PATH');
if isempty(output_path)
    output_path = 'validation/data/spectral_dcm_spm_results.mat';
end
...
try
    fprintf('Running spm_dcm_fmri_csd...\n');
...
fprintf('=== Spectral DCM estimation complete ===\n');
fprintf('End time: %s\n', datestr(now));

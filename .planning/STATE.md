...
**Milestone:** v0.8.0 DCM for Evoked Responses (EEG/MEG ERP). **IN PROGRESS — Phase 33 ✅ COMPLETE; Phase 34 ✅ ALL 3 WAVES COMPLETE; Phase 35 ⏳ Wave 1 DONE (Plan 35-01) — the pure-torch single-dipole lead-field (`L_full = kron(P.J, L_spatial)`, P.J=sp-voltage index 2, column-major = cmc_flatten) + scalp projection + the 4th additive `ERPDCMForward` protocol implementor are GREEN on the laptop, VL-round-trip-confirmed end-to-end, with ZERO engine/protocol/sibling edits.** ROADMAP created (Phases 33-36; 25/25 reqs mapped to one phase each).
**Phase 35 — Single-Dipole Lead-Field, Scalp Projection & ERPDCMForward: 1/3 WAVES DONE (Plan 35-01). Next: Plan 35-02 (Wave 2 M3/MATLAB `spm_lx_erp` LFP lead-field + scalp-ERP fixtures), then Plan 35-03 (Wave 3 scalp parity ladder).**

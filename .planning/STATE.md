...
**Milestone:** v0.8.0 DCM for Evoked Responses (EEG/MEG ERP). **IN PROGRESS — Phase 33 ✅ COMPLETE; Phase 34 ✅ ALL 3 WAVES COMPLETE; Phase 35 ⏳ Waves 1+2 DONE (Plans 35-01, 35-02) — the pure-torch single-dipole lead-field + the frozen SPM12 LFP lead-field/scalp-ERP fixtures are both in hand, and `L_full` is BIT-EXACT between them (max|diff|=0.0).** ROADMAP created (Phases 33-36; 25/25 reqs mapped to one phase each).
**Phase 35 — Single-Dipole Lead-Field, Scalp Projection & ERPDCMForward: 2/3 WAVES DONE (Plans 35-01, 35-02). Next: Plan 35-03 (Wave 3 scalp parity ladder — assert torch build_lead_field/project_to_scalp element-wise vs the frozen fixtures).**

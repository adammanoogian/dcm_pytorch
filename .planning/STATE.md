...
**Milestone:** v0.8.0 DCM for Evoked Responses (EEG/MEG ERP). **IN PROGRESS — Phase 33 ✅ COMPLETE; Phase 34 ✅ ALL 3 WAVES COMPLETE (Plans 34-01, 34-02, 34-03) — the pure-torch CMC network core + C4 dual-B precision mechanism are GREEN on the laptop, the byte-frozen 5-source MMN SPM12 fixtures are generated on M3 + committed, and the EVOK-05 multi-source PARITY GATE PASSES (cmc_network_f IS spm_fx_cmc + apply_condition_modulation IS spm_gen_Q + the trajectory IS spm_gen_erp at N=5).** ROADMAP created (Phases 33-36; 25/25 reqs mapped to one phase each).
**Phase 34 — Extrinsic Coupling, Condition B & Multi-Source Evoked: 3/3 WAVES DONE (Plans 34-01, 34-02, 34-03). Headline parity gate passes at network scale. Next: `/gsd:verify-phase 34` then Phase 35 (single-dipole lead-field + scalp projection).**

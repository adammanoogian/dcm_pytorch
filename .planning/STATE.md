...
**Current focus:** v0.8.0 DCM for Evoked Responses (EEG/MEG ERP) — **✅ ALL 4 PLANNED PHASES (33-36) COMPLETE & VERIFIED.** The CMC neural-mass → evoked → single-dipole lead-field → scalp-ERP forward stack is SPM12-parity-verified at every stage; the ERP-DCM Pyro model + amortized + the gated MMN precision-sweep demo (the actinf_physics hand-off) shipped. 79 ERP tests green on M3 (job 57904695). **Follow-up Phase 37 (ECD frontal-topography) DEFINED, not started — needs user MNI coords.** Milestone ready for `/gsd:audit-milestone` or `/gsd:complete-milestone`. Seed: `.planning/v0.8.0-EEG-ERP-SCOPE.md` + `.planning/research/v0.8.0/`.
...
**36-03 DONE 2026-06-26 (ERPDCM-04 + ERPDCM-06, CAPSTONE demo, LAPTOP — gate+sweep+figure 30.4s):** NEW `scripts/demo_mmn_precision_sweep.py` (gate → permutation guard → pure-forward precision sweep → asserts → publication figure), the `actinf_physics` hand-off artifact. **GATE (ERPDCM-06):** `run_parity_gate()` reproduces the frozen Phase-35 LFP fixture at the fixed-reference P via the reused `_reference_p`/`_production_scalp` path — `L_full` **0.000e+00**, scalp ERP **6.379e-11**, diff wave **1.265e-11**, all ≤1e-7; raises `SystemExit` (missing fixture) / `RuntimeError` (divergence) BEFORE any figure (verified). LAPTOP, no MATLAB. **SWEEP (ERPDCM-04, re-scoped Option A):** sweeps `sp_inhibition_gain` (free `P.G[:,0]`→parameterised `G[:,6]`, permutation guard reused: moved_g6=811>0, moved_g0=0) over [0,1.5] (7 pts) through `simulate_erp_dcm`. **Machine-asserts (1) MONOTONE non-increasing |MMN| transfer curve (1.5385e-5→1.1177e-6) and (2) windowed (90–120ms) MMN minimum NEGATIVE at every gain AND |min| attenuating.** **KEY DECISION 36-03-D1 (Rule-4 STOP → orchestrator Option A):** frontal-dominance is NOT assertable in LFP source-space — across **189 forward-only tunings** best rIFG/A1 ratio = **0.062** (input node A1 dominates raw sp-voltage by ≥16x; ~2000x at baseline) because `mmn_cmc_params` uses the identity LFP lead field; true frontal SCALP dominance is an ECD dipole-orientation phenomenon DEFERRED to a follow-up phase (35-01-D1/Fact 6; `ecd_spatial` exists but needs a nonexistent (Nc,3,n) gain). DROPPED the frontal-dominance + global-negative asserts → RECORDED rIFG/A1 ratio + ECD-deferral note instead (no faking/loosening). **36-03-D2:** negative-going asserted on the MMN-latency-window minimum (the canonical −100ms deflection), not the global |peak| (an early +64ms evoked transient). **36-03-D3:** sweep ∈[0,1.5] (regime where deflection present; beyond ~1.5 sign flips). **FIGURE:** `figures/mmn_precision_sweep.{png,pdf}` (Panel A log-scale gain→|MMN| transfer curve; Panel B rIFG diff-wave overlay gain 0/0.75/1.5 + shaded 90–120ms window) — **gitignored (27-02-D1), script is source of truth**. Citations author/year only (Adams 2013, Ranlund 2016, Garrido 2009, David&Friston 2003, Bastos 2012, Kiebel 2006 — **6 papers flagged for Zotero, NO [REF-xxx] fabricated**). **Additive-only: 1 new script, no shipped module edited; ruff/format clean; mypy delta only the documented `pyro_dcm.*` import-untyped + numpy-stub baseline. LAPTOP 30.4s (<3 min, correctly not routed to M3).** Commits 9300d56 (gate), dfc77f4 (sweep+asserts), 54e765d (figure). See `36-03-SUMMARY.md`. **v0.8.0 CAPSTONE complete; ERPDCM-04 "frontal-dominant" wording amended (windowed-MMN+monotone delivered; frontal scalp topography → ECD phase) — orchestrator to reconcile REQUIREMENTS/ROADMAP + add the ECD phase.**
**36-01 DONE 2026-06-26 (Wave 1, ERP-DCM Pyro model + amortized wiring, LAPTOP unit + M3 amortized fit):** wired the Pyro generative ERP-DCM AND the amortized flow path, REUSING the parity-verified Phase 33/34/35 forward verbatim. NEW `models/erp_dcm_model.py` samples `A_free`(4,N,N)/per-effect `B_free_{j}`(NO `pyro.plate`)/`C_free`/`T`/`G`/`S`/`R` from `spm_cmc_priors.m` log-space Normals (mean 0) + `scalp_noise_scale` (HalfCauchy), masks A/C dead→free `-32` (CMC `exp(P)*E0` dead) and B dead→`0` (additive log-offset), delegates the forward ENTIRELY to `simulate_erp_dcm(..., l_full=...)` (grep-verified NO second `integrate_local_linearization`/`project_to_scalp` — pitfall V1), Gaussian `obs_erp` on the flattened `(Cnd,ns,Nc)` scalp residual. APPENDED `ERPDCMPacker` (identity reshapes, NO `.exp()`; FROZEN order `A_free(4NN)|C_free(NM)|T(4N)|G(4N)|S(N)|R(2M)` == `ERPDCMForward.pack_params`; `n_features==param_count`==38 at N=2,M=1; B EXCLUDED), `ErpSummaryNet` (MLP over flat `(Cnd*ns*Nc,)`→embed, float64), `amortized_erp_dcm_model`+`_run_erp_forward_model` (single `_latent`, reuses `ERPDCMForward.predict` with B FIXED inside `forward` — no amortized scope creep; fixed obs noise since the flow guide samples only `_latent`). `create_guide(auto_normal)` auto-discovers all sites incl. `B_free_0` with ZERO factory edits (MODEL-06). **LAPTOP `-m "not slow"` 6 passed/1 deselected ~11s; M3 amortized-fit slice (the slow jacrev path) job 57903632 COMPLETED exit 0, 7 passed in 91.25s.** **MUTAGEN models/ FOOTGUN CLEARED:** `erp_dcm_model.py` confirmed present on M3 at the models/ path, all 6 changed/new files `md5sum` byte-identical laptop↔M3 after flush (no scp needed). **Additive-only:** Phase 33/34/35 forward stack + `ForwardModel` protocol byte-untouched (`git diff --stat` empty); ruff check clean, appended code canonically format-clean (pre-existing sibling-packer + `amortized_wrappers` `I001` import-sort debt left untouched — pre-existed at HEAD); mypy delta only the numpy-stub baseline. **KEY DECISION (36-01):** B-prior variance stayed PROVISIONAL `1/8` (`B_PRIOR_VARIANCE`; MUST-VERIFY vs `spm_dcm_erp` — `cmc_prior_moments` carries no B) — not load-bearing for the fixed-B headline demo, matters only for B-modulation SVI recovery; flagged in docstring + carried to 36-03. Commits 7bd3b71 (erp_dcm_model), d34caa6 (ERPDCMPacker+ErpSummaryNet), e189011 (amortized+tests+sbatch). See `36-01-SUMMARY.md`. **36-03 inherits a ready ERP-DCM (SVI/VL via `create_guide`) + amortized (`amortized_erp_dcm_model`) path.**
**36-02 DONE 2026-06-26 (ERPDCM-03 + ERPDCM-05, LAPTOP — forward-only/structural, ~8s):** promoted the locked private `_MS_*` 5-source auditory-MMN topology to a PUBLIC importable builder + the forward-only `actinf_physics` Phase-133 adapter, in NEW `forward_models/mmn_reference.py`. **`build_mmn_5source_network()`** returns the canonical MMN graph (A1L=0/A1R=1/STGL=2/STGR=3/rIFG=4) as `{a_masks (4×(5,5) presence: fwd,fwd,bwd,bwd), b_masks ([(5,5)] value: 0.3 edges + 0.5 precision diag), c_mask ((5,1) bilateral A1), x_design ((2,1) std/deviant), source_names, precision_nodes (4,0,1)}`; topology held **byte-identical to the SPM12-parity-gated `_MS_*` fixture** by LAZILY READING those constants from `validation.export_to_mat` (no re-typed edge lists; keeps `forward_models` import scipy-free; locked masks byte-untouched). **`mmn_cmc_params(sp_inhibition_gain,a1_b_gain,rifg_b_gain,fwd_bwd_flag="both")`** builds the free-log-space `p` (`T(5,4)/G(5,4)/C(5,1)/S(5,1)/R(1,2)/A(list[4] (5,5))/B([(5,5)])`) via `_MS_A_LIVE`(0)/`_MS_A_DEAD`(-32), computes the identity LFP lead field, and returns the ready-to-simulate bundle `{p,a_masks,b_masks,c_mask,x_design,l_full}` (feed `simulate_erp_dcm(bundle["p"],bundle["x_design"],5,l_full=bundle["l_full"])`). **KEY (permutation guard reused):** `sp_inhibition_gain` sets the FREE `P.G[node,0]` at `{rIFG,A1L,A1R}` → moves parameterised `G[:,6]` (sp self-inhibition, `J_PERM[0]=6`), NOT `G[:,0]` (never indexed directly). **NO MNI coords** emitted (LFP scope; no-coord-key test guards). 7 LAPTOP tests green (topology-equality element-wise vs `_MS_*`, no-MNI, perm `G6`-not-`G0`, free-`P.G[:,0]`-at-precision-only, adapter bundle finite + non-zero scalp diff wave, fwd/bwd negative control, invalid-flag `ValueError`); 31 ERP-sibling tests green; ruff/format clean; mypy delta only the documented `pyro_dcm.*` import-untyped + numpy-stub baseline. **Additive-only: 1 new module + 1 new test + `__init__` append; staged file-by-file (NEVER touched the parallel 36-01 `models/amortized_wrappers.py` working-tree change); Phase 33/34/35 modules + `export_to_mat.py` byte-untouched.** Commits 937abf9 (builder+adapter+`__init__`), 4818a11 (tests). See `36-02-SUMMARY.md`. **36-03 (MMN precision-sweep demo) unblocked: imports both symbols, sweeps `sp_inhibition_gain` → asserts monotone gain→|MMN| on the non-zero scalp diff wave; `actinf_physics` Phase-133 consumes `mmn_cmc_params` directly.**

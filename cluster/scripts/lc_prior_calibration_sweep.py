...
def run_sweep() -> list[dict]:
    ...
    -------
    list[dict]
        One entry per grid combination with keys: a_prior_var, b_prior_var,
        init_scale, a_rmse, b_rmse, sign_recovery, ci_coverage_95,
        trajectory_r_squared, shrinkage_A, shrinkage_B, final_elbo,
        status, elapsed_s.
    """
...
def print_summary(results: list[dict]) -> None:
    ...
    # Select winner: lowest B-RMSE with shrinkage_B < 0.95.
    valid = [
        r
        for r in ok_results
...
def main() -> None:
    """Run calibration sweep and save results to JSON."""
    results = run_sweep()

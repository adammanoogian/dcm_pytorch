...
def _require_mne() -> None:
    """Raise ImportError if MNE-Python is not installed.

    Raises
    ------
    ImportError
        If ``mne`` cannot be imported, with install instructions.
    """
    try:
...
def epochs_to_timeseries(
    ...
    *,
    ...
    dict
        ...
        - ``'timeseries'``: shape ``(T, N)`` if averaged,
          ``(n_epochs, T, N)`` otherwise.

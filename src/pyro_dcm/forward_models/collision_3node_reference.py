...
# No lateral edge (feedforward/feedback chain); C drives V5/MT only; all three
# nodes carry diag(B) + a per-node precision knob.
_TOPO = CmcTopology(
    n=3,
    source_names=("V5/MT", "SPL/IPS", "PMd"),
    forward=((1, 0), (2, 1)),
    lateral=(),
    backward=((0, 1), (1, 2)),
    inputs=(0,),
...
def collision_3node_cmc_params(
    ...
    * ``fwd_bwd_flag`` -> which extrinsic blocks carry the ``b_edge`` modulation:
    ...
    scalars = read_ms_scalars()
    g_gains = {0: v5_sp_gain, 1: parietal_sp_gain, 2: frontal_sp_gain}

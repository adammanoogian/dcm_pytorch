...
def make_spectral_dcm_simulator(
    ...
    def simulator(theta: torch.Tensor) -> torch.Tensor:
        ...
        # Check for NaN
        if torch.any(torch.isnan(csd.real)) or torch.any(
            torch.isnan(csd.imag)
        ):
            return torch.zeros(output_dim, dtype=torch.float64)
...
def train_npe(
    ...
    embedding_net : nn.Module or None
        ...
        Passed to the neural density estimator via ``posterior_nn``.
    n_rounds : int
    ...
    Uses ``sbi.neural_nets.posterior_nn`` to build the density estimator
    with optional embedding network, and a manual simulation loop
    (``sbi.inference.simulate_for_sbi`` uses ``torch.cat`` which
    concatenates 1D simulator outputs instead of stacking them).
    """
    try:
    ...
    density_estimator = posterior_nn(**nn_kwargs)
    inference = SNPE(
        prior=prior,
    ...
    for _round in range(n_rounds):
        # Manual simulation loop (sbi's simulate_for_sbi uses
        # torch.cat which concatenates 1D outputs instead of stacking)
        theta_list = []
        x_list = []
        for _ in range(n_simulations):
            ...
            x_list.append(x_i)
        theta = torch.stack(theta_list).to(torch.float32)

...
class CSDEmbeddingNet(nn.Module):
    ...
    Two fully-connected layers with ReLU activation and layer
    ...
    Uses ``LayerNorm`` instead of ``BatchNorm1d`` so that the network
    works correctly with batch_size=1 (required by sbi's internal
    shape-probing during neural network initialization).
